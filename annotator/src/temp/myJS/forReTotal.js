...
//千万注意这传的是note的信息
  ...
  function getJsonFromServerAndLoadAllMarks(taskID, user, imgName) {
    ...
    $.ajax({
      type: 'POST',
      url: "mark/checkImage",
      data: JSON.stringify({taskID: taskID, users: [], imgName: imgName}),     //这里查全部传一个空
      // data:JSON.stringify({taskID:123,user:[{username:"a"},{username:"b"}],imgName:"c.png"}),
      success: function (result) {
        console.log(result);
        if (result.marks && result.marks[0]) {
          globalImgMsg.imgURL = result.marks[0].imgURL;
          globalImgMsg.sponsorName = result.marks[0].sponsorName;
          globalImgMsg.taskID = result.marks[0].taskID;
          for(let i = 0; i < result.marks.length; i++){
            let mark = result.marks[i];
            console.log('检视一个noteRect');
            console.log(mark.noteRectangle);
            globalImgMsg.noteRectangle= globalImgMsg.noteRectangle.concat(mark.noteRectangle);
            globalImgMsg.notePolygon = globalImgMsg.notePolygon.concat(mark.notePolygon);
          }
